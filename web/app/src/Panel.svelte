<script>
  ...
  import { versionLabel, versionInfo, VERSION, REPO_URL } from "./lib/version.js";

  let git = $state(null);
  versionInfo().then(v => (git = v?.git ?? null));
...
<aside class="panel" aria-label="navigation">
  ...
  <div class="scroll">
    ...
    <p class="version">pisynth {versionLabel(VERSION, git)}{DEMO ? " · demo" : ""} · <a href={REPO_URL} target="_blank" rel="noopener">GitHub</a></p>
  </div>
</aside>
...
<style>
  ...
  .version { margin-top: 8px; text-align: center; font-size: .72rem; color: var(--muted); }
  .version a { color: var(--muted); text-decoration: underline; }
