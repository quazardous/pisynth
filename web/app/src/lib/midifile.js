...
export function parseMidi(buffer) {
  ...
  for (let t = 0; t < ntracks && p < v.byteLength; t++) {
    ...
    while (p < end) {
      ...
      if (kind === 0x90 || kind === 0x80 || (kind === 0xb0 && d1 === 64)) raw.push({ tick, order: order++, kind: "midi", status, d1, d2, track: t });
    }
    p = end;
  }
...
export function sampleSong(bpm = 100) {
  ...
  const note = (t, n, len, vel = 90, track = 0) => { ev.push({ ms: t, status: 0x90, d1: n, d2: vel, track }); ev.push({ ms: t + len, status: 0x80, d1: n, d2: 0, track }); };
  ...
  for (const chord of [[48, 60, 64, 67], [45, 57, 60, 64], [41, 57, 60, 65], [43, 55, 59, 62]]) {
    ...
    chord.forEach((n, i) => note(t + i * 15, n, beat * 2 - 40, 80, n < 60 ? 1 : 0));
    ev.push({ ms: t + beat * 2 - 10, status: 0xb0, d1: 64, d2: 0 });
    t += beat * 2;
  }
  ev.sort((a, b) => a.ms - b.ms);
  return { name: "Sample: scale + I–vi–IV–V", events: ev, durationMs: t, bpm };
}
